...
--~		1 ---> task, 2 --> matchpos, 3--->kick, 4 --->dir, 5 --->pre, 6 --->kp, 7--->cp, 8--->flag
		kickStatus:clearAll()
		for rolename, task in pairs(curPlay[gRealState]) do
			if(type(task) == "table" and rolename ~= "match" and gRoleNum[rolename] ~= nil) then
--~ 			统一进行射门和吸球的管理,并做任务处理
				if task[1] == nil then
				...
				if roleNum ~= 0 then
					...
					debugEngine:gui_debug_msg(vision:OurPlayer(gRoleNum[rolename]):Pos(), string.sub(rolename, 1, 1))
					task[1](roleNum)
				end
			end
		end
