...
["prepare"] = {
	switch = function ()
		print("prepare")
		if bufcnt(player.toPointDist("Kicker",TargetPos)<20 and ball.posX()<player.posX("Kicker")-30, "slow") then
			return "wait"
		end
	end,
...
["wait"] = {
	switch = function ()
		print("wait")
		if bufcnt(ball.posX()>player.posX("Kicker")-15 and ball.velMod()>20 and ball.valid(), "fast")then
			return "chase"
		end
	end,
	Kicker  = task.goCmuRush(TargetPos),
