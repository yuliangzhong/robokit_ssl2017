local FINAL_SHOOT_POS  = CGeoPoint:new_local(270,-240)
...
["startball"] = {
	...
	Middle   = task.goCmuRush(ANTI_CORNER_POS1, dir.playerToBall),
	Defender = task.singleBack(),
	Goalie   = task.goalie(),
	match    = "{A}[LSM][D]"
},
...
["sidegoto"] = {
	switch = function ()
		if bufcnt(player.toTargetDist("Middle") < 110, "fast", 150) then
			return "sidepass"
		end
	end,
...
["sidepass"] = {
	...
	Assister = USE_CHIP and task.chipPass(CHIP_POS, 250) or task.goAndTurnKick(CHIP_POS, 560),
	Leader   = task.goCmuRush(SYNT_CORNER_POS1),
	Special  = task.goCmuRush(SYNT_CORNER_POS2),
	Middle   = task.goCmuRush(ANTI_CORNER_POS1, dir.playerToBall),
	Defender = task.singleBack(),
