...
-- 大场：
local AtrPos1  = ball.refAntiYPos(CGeoPoint:new_local(180,220))    --拉人位置一
local AtrPos2  = ball.refAntiYPos(CGeoPoint:new_local(300,280))    --拉人位置二
local TMP_DEF_POS1 = ball.refAntiYPos(CGeoPoint:new_local(-200,200))
local TMP_DEF_POS2 = ball.refAntiYPos(CGeoPoint:new_local(100,120))
local chipPos = CGeoPoint:new_local(325,-140)
local ChipPos = ball.refAntiYPos(chipPos)
...
gPlayTable.CreatePlay{
	...
	["pass"] = {
		...
		Kicker   = task.chipPass(chipPos,110),
	...
	["approch"] = {
		switch = function ()
			if bufcnt(ball.posX() - player.posX("Assister") > 30, "normal", 60) then
				return "chase"
			end
		end,
