...
local FINAL_SHOOT_POS = CGeoPoint:new_local(265,208)
...
gPlayTable.CreatePlay{
  ...
  ["passBall"] = {
    ...
    Assister = USE_CHIP and task.chipPass(CHIP_POS, 5555+90) or task.goAndTurnKick(CHIP_POS, 5555+30),
  ...
  ["waitBall"] = {
    switch = function ()
      if bufcnt(player.toPointDist("Leader", TMP_SHOOT_POS[3]()) < 30, 3,80) then
        return "shootBall"
      end
    end,
  ...
  ["shootBall"] = {
    ...
    Assister = task.leftBack(),
    Leader   = USE_CHIP and task.waitTouch(ball.refAntiYPos(FINAL_SHOOT_POS), 0) or task.touch(),
