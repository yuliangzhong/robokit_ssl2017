...
local RUSH_POS = ball.goRush()
local PASS_POS = pos.passForTouch(RUSH_POS)
...
if useLearnRound then
  ...
  TURN_KICK    = task.passToPos(RUSH_POS,200)
end
...
["goThreeJam"] = {
  switch = function ()
    if bufcnt(player.toTargetDist("Defender") < 20, 10, 120) then
...
["goKick"] = {
  ...
  Breaker  = task.touch(),
  Assister = task.goPassPos("Breaker", flag.allow_dss),
