...
local LEFTSIDE_POS2 = function()
	if ball.refAntiY() == -1 then
		return CGeoPoint:new_local(-300,0)
	else 
		return CGeoPoint:new_local(-320,0)
	end
end
...
local RIGHTSIDE_POS2 = function()
	if ball.refAntiY() == -1 then
		return CGeoPoint:new_local(-320, param.pitchWidth/2)
	else 
		return CGeoPoint:new_local(-300, param.pitchWidth/2)
	end
end
