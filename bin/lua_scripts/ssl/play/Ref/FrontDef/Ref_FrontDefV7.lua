...
--针对MRL直接挑前场
local BorderUpRight = -0
local BorderDownRight = -200
local BorderUpLeft = 80
local BorderDownLeft = -180
...
switch = function()
	...
	elseif gCurrentState == "rightAttackDef" then
		--print(cond.canExitFrontDef(),cond.needExitAttackDef(RIGHTSIDE_POS1(),RIGHTSIDE_POS2(),"horizal"))
		if bufcnt((cond.canExitMRLFrontDef() and ball.valid())or cond.needExitAttackDef(RIGHTSIDE_POS1(),RIGHTSIDE_POS2(),"horizal") or cond.canDefenceExit(), 2, 100) then
			return "finish"
	...
	elseif gCurrentState == "leftAttackDef" then
		...
		if bufcnt((cond.canExitMRLFrontDef() and ball.valid()) or cond.needExitAttackDef(LEFTSIDE_POS1(),LEFTSIDE_POS2(),"horizal") or cond.canDefenceExit(), 2, 100) then
			return "finish"
		elseif bufcnt(cond.canBeImmortalShoot(),8) then
			return "finish"
		end
