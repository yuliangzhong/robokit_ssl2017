...
local FINAL_SHOOT_POS = CGeoPoint:new_local(360, 120)
...
local CHIP_POS	  = pos.passForTouch(CGeoPoint:new_local(363, 80))
...
["start"] = {
	...
	Special  = task.goCmuRush(ball.refAntiYPos(CGeoPoint:new_local(180, 160))),
	Middle   = task.goCmuRush(AVOID_KEEPER_POS),
	Defender = task.goCmuRush(READY_POS2),
	Leader   = task.goCmuRush(READY_POS3),
	Goalie   = task.goalie(),
	match    = "{A}{LSMD}"
},
...
["gopass"] = {
	...
	Assister = task.chipPass(CHIP_POS, 5555+35),
	Middle   = task.goCmuRush(RUSH_POS1),
	Defender = task.goCmuRush(RUSH_POS2),
