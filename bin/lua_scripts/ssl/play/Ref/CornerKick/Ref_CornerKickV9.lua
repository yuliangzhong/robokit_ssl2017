...
local FINAL_SHOOT_POS = CGeoPoint:new_local(330, 90)
...
["tmpState"] = {
	switch = function()
		if bufcnt(player.toTargetDist("Middle") < 90, "normal", 180) then
			return "gotoState"
		end
	end,
...
["gotoState"] = {
	switch = function()
		if bufcnt(player.toTargetDist("Middle") < 90, "fast", 120) then
			return "chipBall"
		end
	end,
	Assister = USE_CHIP and task.slowGetBall(CHIP_POS) or task.staticGetBall(CHIP_POS),
...
["chipBall"] = {
	...
	Assister = USE_CHIP and task.chipPass(CHIP_POS, 5555+90) or task.goAndTurnKick(CHIP_POS, 5555+70),
	Leader   = task.goCmuRush(END_POS1, _, 600, flag.allow_dss),
	Special  = task.goCmuRush(END_POS2, _, 600, flag.allow_dss),
...
["kickBall"] = {
	...
	Special  = task.waitTouch(END_POS2, 0),
