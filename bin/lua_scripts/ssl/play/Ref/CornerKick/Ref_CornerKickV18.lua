...
gPlayTable.CreatePlay{
  ...
  ["start"] = {
    ...
    Special  = task.goCmuRush(SYNT_POS_2),
    Middle   = task.goCmuRush(SYNT_POS_3),
    Defender = task.goCmuRush(SYNT_POS_4),
    Goalie   = task.goalie(),
    match    = "{A}{LDMS}"
  },
  ...
  ["runTmpPos"] = {
    ...
    Middle   = task.goCmuRush(SYNT_POS_3),
    Defender = task.goCmuRush(SYNT_POS_4),
    Goalie   = task.runMultiPos(TMP_SHOOT_POS, false, 40, _, 500),
    match    = "{ALDMS}"
  },
  ...
  ["goalieBack"] = {
    ...
    Middle   = task.rightBack(),
    Defender = task.leftBack(),
    Goalie   = task.goCmuRush(CGeoPoint:new_local(-400, 0), 3.14),
    match    = "[AL][DMS]"
  },
