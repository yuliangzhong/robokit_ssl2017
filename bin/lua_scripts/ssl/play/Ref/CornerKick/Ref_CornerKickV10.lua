...
local FINAL_SHOOT_POS = CGeoPoint:new_local(320,100)
local CHIP_POS  = pos.passForTouch(CGeoPoint:new_local(320,-100))
local SHOOT_POS = ball.refSyntYPos(FINAL_SHOOT_POS)
local RUN_POS = ball.refAntiYPos(CGeoPoint:new_local(220,110))
...
gPlayTable.CreatePlay{
  ...
  ["goshoot"] = {
    switch = function ()
      if bufcnt(player.toTargetDist("Leader") < 80, "fast", 120) then
        return "pass"
      end
    end,
    Assister = task.slowGetBall(CHIP_POS),
    Leader   = task.goCmuRush(SHOOT_POS, player.toBallDir),
    Special  = task.goCmuRush(JAM_POS2),
    Middle   = task.goCmuRush(JAM_POS3),
    Defender = task.goCmuRush(JAM_POS4),
    Goalie   = task.goalie(),
  ...
  ["touch"] = {
    ...
    Assister = task.goSupportPos("Leader"),
    Leader   = task.Intercept(),--task.waitTouch(SHOOT_POS, 0),
    Middle   = task.leftBack(),
    Special  = task.rightBack(),
    Defender = task.defendMiddle(),
    Goalie   = task.goalie(),
