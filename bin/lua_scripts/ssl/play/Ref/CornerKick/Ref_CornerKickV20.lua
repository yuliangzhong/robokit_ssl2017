...
gPlayTable.CreatePlay{
  ...
  ["runUpPos1"] = {
    switch = function ()
      if  bufcnt(player.toTargetDist("Defender") < 20 ,20,60) then
        return "runUpPos2"
      end
    end,
  ...
  ["runUpPos3"] = {
    switch = function ()
     if player.toPointDist("Defender", TMP_SHOOT_POS[2]()) < 30 then
        return "passBall"
      end
    end,
    Assister = USE_CHIP and task.slowGetBall(PASS_POS) or task.staticGetBall(PASS_POS),
  ...
  ["waitBall"] = {
    switch = function ()
         if bufcnt(player.toPointDist("Defender", TMP_SHOOT_POS[3]()) < 30, "fast", 60) then
        return "shootBall"
      end
    end,
    Assister = task.stop(),
    ...
    Special  = task.goCmuRush(SPECIAL_POS, _, 600, flag.allow_dss),
    Middle   = task.rightBack(),
    Defender = task.waitTouch(TMP_SHOOT_POS[3], 0),
    Goalie   = task.goalie(),
    match    = "{ALDMS}"
  },
