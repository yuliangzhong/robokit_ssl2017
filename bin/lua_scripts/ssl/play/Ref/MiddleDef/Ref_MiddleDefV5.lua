--marking touch, 无逼抢机制
--如果对方5车上来就可能会漏人，针对对方横传球
--如果对方5车都上来，就用v2
local LEFTUP_POS = function()
	if ball.refAntiY() == -1 then
		return CGeoPoint:new_local(300, 280*ball.refAntiY())
	else
		return CGeoPoint:new_local(300, 90*ball.refAntiY())
	end
end
...
local RIGHTDOWN_POS = function()
	if ball.refAntiY() == -1 then
		...
		return CGeoPoint:new_local(-30, 280*ball.refAntiY())
	end
end
