...
namespace rbk {
    namespace utils {
        namespace udp {
            class RBK_DLL_EXPORT AsyncUDPClient {
            public:
                AsyncUDPClient();
            ...
            private:
                ...
                boost::atomic<bool> error; ///< Error flag

                /// Read complete callback
                boost::function<void(const char*, size_t)> _callback;
            };

            // TODO, modify when use C++11
            typedef boost::asio::basic_waitable_timer<boost::chrono::steady_clock> steadyTimer;

            class RBK_DLL_EXPORT SyncUDPClient {
            public:
                /**
                * Default Constructor.
                */
