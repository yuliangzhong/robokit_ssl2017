...
namespace DefendUtils{
	...
	double GoalBuffer = 2;
	double PLAYERSIZE = Param::Vehicle::V2::PLAYER_SIZE - 1.0; //本篇所用的player size  0.5
  ...
  CGeoEllipse RD_ELLIPSE1 = CGeoEllipse(CENTERPOINT, Param::Field::PENALTY_AREA_DEPTH + 15 + AVOIDBUFFER, (Param::Field::PENALTY_AREA_WIDTH + 50 + AVOIDBUFFER*2)/2);
	CGeoEllipse RD_ELLIPSE2 = CGeoEllipse(CENTERPOINT, Param::Field::PENALTY_AREA_DEPTH + 10 + AVOIDBUFFER, (Param::Field::PENALTY_AREA_WIDTH + 50 + AVOIDBUFFER*2)/2);
	//CGeoEllipse RD_ELLIPSE2 = CGeoEllipse(CENTERPOINT, Param::Field::PENALTY_AREA_DEPTH + 5  + AVOIDBUFFER, (Param::Field::PENALTY_AREA_WIDTH + 8 + AVOIDBUFFER*2)/2);
  ...
  CGeoPoint DCENTERPOINT = CGeoPoint(-Param::Field::PITCH_LENGTH/2,0);
	...
	CGeoPoint calcDefenderPointV2(const CGeoPoint Rtarget,const double Rdir,const posSide Rside,int mode,double ratio){
		...
		//当ratio为-1是后卫调用点，否则为盯人和防头球调用点
			...
			if (mode == 1){
				CVector transVector = Utils::Polar2Vector(PLAYERSIZE,Utils::Normalize(Rbasedir + sideFactor * Param::Math::PI / 2));
				CGeoPoint transPoint = Rbasepoint + transVector;
				if (RdefenderPoint.x()>425){
					//have modified by thj
					if (transPoint.y()<0){
						RdefenderPoint = CGeoPoint(425,-155);
					}else{
						RdefenderPoint = CGeoPoint(425,155);
					}
				}
	...
	bool checkInDeadArea(){
		...
		bool result = false;
		//球在死角区域内返回true
		if ((!Utils::InOurPenaltyArea(pVision->Ball().Pos(),30) && x >-Param::Field::PITCH_LENGTH / 2.0 && x<-300 &&
			(y - 2*x - 900)>0 && y >0 && y<Param::Field::PITCH_WIDTH / 2.0)||
			(!Utils::InOurPenaltyArea(pVision->Ball().Pos(),30) && x >-Param::Field::PITCH_LENGTH / 2.0 && x<-300 &&
			(y + 2*x + 900)<0 && y <0 && y>-Param::Field::PITCH_WIDTH / 2.0)){
				result = true;
		}
		return result;
