...
void CSupportPos::generatePos(const CVisionModule* pVision,int num){
	...
	/*if ((ourLeader.Pos() - ball.Pos()).mod() > (ourSupporter.Pos() - ball.Pos()).mod()-30){
		_roleChangeCounter++;
	}
	else{
		_roleChangeCounter = 0;
	}
	if (_roleChangeCounter == 12){
		if ((ourLeader.Pos() - ball.Pos()).mod() > (ourSupporter.Pos() - ball.Pos()).mod()-30){
			_supportPos = ball.Pos();
			cout << "6666" << endl;
		}
		else{
			_roleChangeCounter = 0;
	...
	_lastPos = _supportPos;
	//keepLeastDistanceFromLeader(pVision, 150);
	/*if (pVision->Cycle() - _savedCycle > 120){
		if (_roleChangeCounter > 3){
			isNeedStay = true;
		}
		else{
			isNeedStay = false;
