...
void CWaitTouch::plan(const CVisionModule* pVision) 
{
	...
	if (car_2_ball.mod() < 20 && ball.Pos().x() > 0) {
		KickStatus::Instance()->setKick(runner, 1300); 
	}
	if (!need_shoot) {
		...
		GDebugEngine::Instance()->gui_debug_msg(wait_kick_task.player.pos, "here", COLOR_CYAN);
		...
		GDebugEngine::Instance()->gui_debug_msg(wait_kick_task.player.pos, "touch", COLOR_CYAN);
	}
