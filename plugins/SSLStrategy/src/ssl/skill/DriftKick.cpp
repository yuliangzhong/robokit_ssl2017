...
void CDriftKick::plan(const CVisionModule* pVision)
{
	...
	if (ballSpeed>=Ball_Moving_Fast_Speed)
	{
		...
		case SPEED_UP:
			...
			//cout<<"speed Up"<<endl;
			speedUpDistanceY=max<double>(kickPos.y()-me.Pos().y(),0)+crossWiseFactor[realNum]*Param::Vehicle::V2::PLAYER_SIZE+max<double>(ballSpeed-130,0)*sin(fabs(ballVelDir))*0.2;
			...
			speedUpDistanceX=max(speedUpDistanceX,-5.0);
