...
void CMarking::plan(const CVisionModule* pVision)
{
	...
	*****************************/
	...
	if (needToUpdateTheirKicker == true) {
		if (ball.Vel().mod() < 10) {
			double minDist = pVision->TheirPlayer(1).Pos().dist(ball.Pos());
			for (int i = 2; i <= Param::Field::MAX_PLAYER; ++i) {
				const PlayerVisionT& theirPlayer = pVision->TheirPlayer(i);
				if (theirPlayer.Pos().dist(ball.Pos()) < minDist) {
					minDist = theirPlayer.Pos().dist(ball.Pos());
					theirKicker = &theirPlayer;
				}
			}
		} else {
			needToUpdateTheirKicker = false;
		}
	}
